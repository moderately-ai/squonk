...
///
/// Reached via [`parse_with`](crate::parse_with), e.g. `parse_with(src, Mssql)`. MSSQL is
...
mod tests {
    ...
    fn mssql_parses_cross_and_outer_apply_that_every_oracle_preset_rejects() {
        ...
        for sql in [
            ...
            assert!(parse_with(sql, Mssql).is_ok(), "MSSQL parses {sql:?}");
    ...
    fn mssql_quotes_identifiers_with_brackets_and_double_quotes() {
        ...
        assert!(
            parse_with("SELECT \"col\" FROM t", Mssql).is_ok(),
    ...
    fn mssql_parses_national_string_literals() {
        ...
        assert!(
            parse_with(national, Mssql).is_ok(),
            "MSSQL reads `N'hi'` as a national-character string",
        );
        assert!(
            parse_with(national, Sqlite).is_ok(),
