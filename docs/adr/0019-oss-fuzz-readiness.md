...
The fuzzing *itself* was ready in design; the blockers were about audience and ownership, not the harness. Two of the four ADR-0014 criteria were green (licensing, target design), and two were not met (a demonstrated crash-free record, and — decisively — an external user base plus a committed triage owner). At the decision date, for an alpha (`v0.1.0`) parser with a single internal consumer, the cost of a continuously-running external service that files deadline-bearing bugs exceeded the benefit, especially while the same targets could already be fuzzed locally and later in CI. This kept the ADR-0014 deferral deliberate and visible rather than forgotten.

## Readiness criteria at the decision date (ADR-0014)
...
| **Maintenance capacity + user base** | At the 2026-06-29 decision, the libraries were pre-1.0 and the sole known downstream was internal; there was **no external/public user base**. No maintainer had committed to triaging OSS-Fuzz reports inside the 90-day disclosure window — and the `differential` target in particular would generate ongoing triage load. This was the decisive factor and matched ADR-0014's stated reason. | **Not met** |
