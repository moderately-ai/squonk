...
The selectable dialects are the value list `BuiltinDialect::ALL`. `Ansi` — the always-compiled SQL-standard baseline that `parse` defaults to — is joined by the feature-gated presets `Postgres`, `MySql`, `Sqlite`, `DuckDb`, `QuiltDb`, `BigQuery`, `Hive`, `ClickHouse`, `Databricks`, `Mssql`, `Snowflake`, `Redshift`, and the permissive `Lenient` "parse-anything" union; `full` turns on every feature-gated dialect.

Each preset carries a release-contract **support tier** recording how strong its parity evidence is; the authoritative per-dialect tier, source of truth, and oracle kind are generated into [`docs/support-tiers.md`](support-tiers.md) from `BuiltinDialect::support_tier` / `support_evidence`. In brief: `Ansi`, `Postgres`, `MySql`, `Sqlite`, `DuckDb`, and `QuiltDb` are **stable**, each held to an authoritative source of truth (an engine differential, the SQL standard for `Ansi`, or a frozen first-party parser contract); `BigQuery`, `ClickHouse`, and `Lenient` are **preview**; and `Hive`, `Databricks`, `Mssql`, `Snowflake`, and `Redshift` are **experimental** documentation-derived presets with no differential oracle wired. The conservative ANSI-derived presets enable only the surface that already has a modelled, tested parser gate and reject unmodelled syntax cleanly.
...
- **JavaScript and TypeScript** — `squonk-node` provides the fast Node-API
  engine, while `squonk-wasm` covers Deno, browsers, workers, edge runtimes,
  and the no-addons fallback. One typed TypeScript facade sits over both.
...
| `crates/squonk-wasm` | `squonk-wasm` | no | `wasm32` bindings exposing the pure-Rust parser to JS / the browser / the edge. Normal parses return opaque WASM-owned document handles, so parse → render stays native; JSON is materialized only when a caller requests `raw`, `toJSON()`, or `parseJson`. The facade provides strict TypeScript declarations, typed `Document` / `Node` / `Ident` / `ObjectName` / `Diagnostic` wrappers, generated AST field metadata, discriminated token/trivia output, rendering, redaction, transpilation, and Node/browser examples. Built on the pure-Rust published crates (ADR-0017), it compiles to `wasm32-unknown-unknown` with no C toolchain — for in-browser linters/formatters, LSP-in-the-browser, and edge SQL validation. |
| `crates/squonk-node` | `squonk-node` | no | Shared Node-API 8 backend for the npm facade packages. It exposes opaque Rust-owned document handles so parse → render avoids JSON materialization, and is released as eight script-free optional platform packages for macOS, Linux glibc/musl, and Windows on x64/arm64. |
| `crates/squonk-python` | `squonk-python` | no | Python bindings — a maturin-packaged `squonk._native` extension module plus pure-Python typed views. Normal parses return opaque Rust-owned document handles and release the GIL during parsing, so parse → render stays native; serde JSON is materialized only for structured Python access or the explicit `parse_dict` / `parse_recovering_dict` helpers. The public surface includes `Document` / `Node` / `Ident` / `ObjectName` / `Diagnostic` / `Trivia` wrappers, `TypedDict` JSON shapes, generated AST field metadata, discriminated token/trivia output, rendering, redaction, transpilation, and runnable examples. |
