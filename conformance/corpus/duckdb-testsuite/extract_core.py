...
#!/usr/bin/env python3
"""Extract a core-tranche DuckDB SQL corpus (accepts + rejects) from the vendored
test suite's sqllogictest-style `.test` files.

Unlike the signature-weighted `conformance/corpus/duckdb/extract.py`, this draws a
*broad* slice of a curated set of core directories — select/join/subquery/aggregate/
window/cte/order/limit/types — to measure the true grammar-gap inventory the
signature weighting skipped. Three artifacts:

  python3 extract_core.py <duckdb_checkout> statements.sql statements_with_schema.sql rejects.sql [--cap N] [--reject-cap M] [--count]
...
CORE_DIRS = ["select", "join", "subquery", "aggregate", "window", "cte", "order", "limit", "types"]
