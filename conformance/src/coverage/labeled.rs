...
const TRANSACTION_SUBFLAGS: &[&str] = &[
    ...
    "transaction_work_keyword",
    "begin_transaction_keyword",
    "commit_transaction_keyword",
    "rollback_transaction_keyword",
    "begin_transaction_modes",
    ...
    "abort_transaction_alias",
    "end_transaction_alias",
    "transaction_release",
    "transaction_chain",
...
const COMPOSITE_SUBFLAGS: &[(Feature, &[&str])] = &[
    ...
    (Feature::TransactionSyntax, TRANSACTION_SUBFLAGS),
...
toggleable_features! {
    ...
    (CREATE_OR_REPLACE_TABLE, "create_or_replace_table", StatementDdlGates, statement_ddl_gates, StatementDdlGates, create_or_replace_table),
    ...
    (START_TRANSACTION, "start_transaction", TransactionSyntax, transaction_syntax, TransactionSyntax, start_transaction),
    (START_TRANSACTION_BLOCK_OPTIONAL, "start_transaction_block_optional", TransactionSyntax, transaction_syntax, TransactionSyntax, start_transaction_block_optional),
    (TRANSACTION_WORK_KEYWORD, "transaction_work_keyword", TransactionSyntax, transaction_syntax, TransactionSyntax, transaction_work_keyword),
    (TRANSACTION_NAME, "transaction_name", TransactionSyntax, transaction_syntax, TransactionSyntax, transaction_name),
    (BEGIN_TRANSACTION_KEYWORD, "begin_transaction_keyword", TransactionSyntax, transaction_syntax, TransactionSyntax, begin_transaction_keyword),
    (COMMIT_TRANSACTION_KEYWORD, "commit_transaction_keyword", TransactionSyntax, transaction_syntax, TransactionSyntax, commit_transaction_keyword),
    (ROLLBACK_TRANSACTION_KEYWORD, "rollback_transaction_keyword", TransactionSyntax, transaction_syntax, TransactionSyntax, rollback_transaction_keyword),
    (BEGIN_TRANSACTION_MODES, "begin_transaction_modes", TransactionSyntax, transaction_syntax, TransactionSyntax, begin_transaction_modes),
    (TRANSACTION_SAVEPOINTS, "transaction_savepoints", TransactionSyntax, transaction_syntax, TransactionSyntax, transaction_savepoints),
    (SET_TRANSACTION, "set_transaction", TransactionSyntax, transaction_syntax, TransactionSyntax, set_transaction),
    (TRANSACTION_ISOLATION_MODE, "transaction_isolation_mode", TransactionSyntax, transaction_syntax, TransactionSyntax, transaction_isolation_mode),
    (TRANSACTION_ACCESS_MODE, "transaction_access_mode", TransactionSyntax, transaction_syntax, TransactionSyntax, transaction_access_mode),
    (TRANSACTION_DEFERRABLE_MODE, "transaction_deferrable_mode", TransactionSyntax, transaction_syntax, TransactionSyntax, transaction_deferrable_mode),
    (START_TRANSACTION_ISOLATION_MODE, "start_transaction_isolation_mode", TransactionSyntax, transaction_syntax, TransactionSyntax, start_transaction_isolation_mode),
    (START_TRANSACTION_DEFERRABLE_MODE, "start_transaction_deferrable_mode", TransactionSyntax, transaction_syntax, TransactionSyntax, start_transaction_deferrable_mode),
    (START_TRANSACTION_CONSISTENT_SNAPSHOT, "start_transaction_consistent_snapshot", TransactionSyntax, transaction_syntax, TransactionSyntax, start_transaction_consistent_snapshot),
    (TRANSACTION_MULTIPLE_MODES, "transaction_multiple_modes", TransactionSyntax, transaction_syntax, TransactionSyntax, transaction_multiple_modes),
    (TRANSACTION_MODES_REQUIRE_COMMAS, "transaction_modes_require_commas", TransactionSyntax, transaction_syntax, TransactionSyntax, transaction_modes_require_commas),
    (TRANSACTION_MODES_REJECT_DUPLICATES, "transaction_modes_reject_duplicates", TransactionSyntax, transaction_syntax, TransactionSyntax, transaction_modes_reject_duplicates),
    (ABORT_TRANSACTION_ALIAS, "abort_transaction_alias", TransactionSyntax, transaction_syntax, TransactionSyntax, abort_transaction_alias),
    (END_TRANSACTION_ALIAS, "end_transaction_alias", TransactionSyntax, transaction_syntax, TransactionSyntax, end_transaction_alias),
    (TRANSACTION_RELEASE, "transaction_release", TransactionSyntax, transaction_syntax, TransactionSyntax, transaction_release),
    (TRANSACTION_CHAIN, "transaction_chain", TransactionSyntax, transaction_syntax, TransactionSyntax, transaction_chain),
    (RELEASE_SAVEPOINT_KEYWORD_OPTIONAL, "release_savepoint_keyword_optional", TransactionSyntax, transaction_syntax, TransactionSyntax, release_savepoint_keyword_optional),
    ...
    (BEGIN_TRANSACTION_MODE, "begin_transaction_mode", TransactionSyntax, transaction_syntax, TransactionSyntax, begin_transaction_mode),
    (XA_TRANSACTIONS, "xa_transactions", TransactionSyntax, transaction_syntax, TransactionSyntax, xa_transactions),
...
const LABELED_CASES: &[LabeledCase] = &[
    ...
    // START with no block word: when the optional-block flag is off, the parser requires
    // the standard TRANSACTION word.
    LabeledCase {
        sql: "START",
        expect: Expect::Accept,
        required: &[&START_TRANSACTION, &START_TRANSACTION_BLOCK_OPTIONAL],
        forbidden: &[],
    },
    LabeledCase {
        sql: "BEGIN WORK",
        expect: Expect::Accept,
        required: &[&TRANSACTION_WORK_KEYWORD],
        forbidden: &[],
    },
    LabeledCase {
        sql: "BEGIN TRANSACTION",
        expect: Expect::Accept,
        required: &[&BEGIN_TRANSACTION_KEYWORD],
        forbidden: &[],
    },
    LabeledCase {
        // SQLite optional transaction name after an explicit TRANSACTION block word.
        sql: "BEGIN TRANSACTION txn",
        expect: Expect::Accept,
        required: &[&BEGIN_TRANSACTION_KEYWORD, &TRANSACTION_NAME],
        forbidden: &[],
    },
    LabeledCase {
        sql: "COMMIT TRANSACTION",
        expect: Expect::Accept,
        required: &[&COMMIT_TRANSACTION_KEYWORD],
        forbidden: &[],
    },
    LabeledCase {
        sql: "ROLLBACK TRANSACTION",
        expect: Expect::Accept,
        required: &[&ROLLBACK_TRANSACTION_KEYWORD],
        forbidden: &[],
    },
    LabeledCase {
        sql: "BEGIN READ ONLY",
        expect: Expect::Accept,
        required: &[&BEGIN_TRANSACTION_MODES, &TRANSACTION_ACCESS_MODE],
        forbidden: &[],
    },
    LabeledCase {
        sql: "SAVEPOINT s",
        expect: Expect::Accept,
        required: &[&TRANSACTION_SAVEPOINTS],
        forbidden: &[],
    },
    LabeledCase {
        sql: "SET TRANSACTION READ ONLY",
        expect: Expect::Accept,
        required: &[&SET_TRANSACTION, &TRANSACTION_ACCESS_MODE],
        forbidden: &[],
    },
    LabeledCase {
        sql: "START TRANSACTION ISOLATION LEVEL SERIALIZABLE",
        expect: Expect::Accept,
        required: &[&START_TRANSACTION, &START_TRANSACTION_ISOLATION_MODE],
        forbidden: &[],
    },
    LabeledCase {
        sql: "START TRANSACTION READ ONLY",
        expect: Expect::Accept,
        required: &[&START_TRANSACTION, &TRANSACTION_ACCESS_MODE],
        forbidden: &[],
    },
    LabeledCase {
        sql: "START TRANSACTION DEFERRABLE",
        expect: Expect::Accept,
        required: &[&START_TRANSACTION, &START_TRANSACTION_DEFERRABLE_MODE],
        forbidden: &[],
    },
    LabeledCase {
        sql: "SET TRANSACTION ISOLATION LEVEL SERIALIZABLE",
        expect: Expect::Accept,
        required: &[&SET_TRANSACTION, &TRANSACTION_ISOLATION_MODE],
        forbidden: &[],
    },
    LabeledCase {
        sql: "SET TRANSACTION DEFERRABLE",
        expect: Expect::Accept,
        required: &[&SET_TRANSACTION, &TRANSACTION_DEFERRABLE_MODE],
        forbidden: &[],
    },
    LabeledCase {
        sql: "START TRANSACTION WITH CONSISTENT SNAPSHOT",
        expect: Expect::Accept,
        required: &[&START_TRANSACTION, &START_TRANSACTION_CONSISTENT_SNAPSHOT],
        forbidden: &[],
    },
    LabeledCase {
        sql: "SET TRANSACTION READ ONLY ISOLATION LEVEL SERIALIZABLE",
        expect: Expect::Reject,
        required: &[&TRANSACTION_MODES_REQUIRE_COMMAS],
        forbidden: &[],
    },
    LabeledCase {
        sql: "SET TRANSACTION READ ONLY, READ WRITE",
        expect: Expect::Reject,
        required: &[&TRANSACTION_MODES_REJECT_DUPLICATES],
        forbidden: &[],
    },
    LabeledCase {
        sql: "START TRANSACTION READ ONLY, READ WRITE",
        expect: Expect::Accept,
        required: &[
            &START_TRANSACTION,
            &TRANSACTION_ACCESS_MODE,
    ...
    LabeledCase {
        sql: "ABORT",
        expect: Expect::Accept,
        required: &[&ABORT_TRANSACTION_ALIAS],
        forbidden: &[],
    },
    LabeledCase {
        sql: "END",
        expect: Expect::Accept,
        required: &[&END_TRANSACTION_ALIAS],
        forbidden: &[],
    },
    LabeledCase {
        sql: "COMMIT AND CHAIN",
        expect: Expect::Accept,
        required: &[&TRANSACTION_CHAIN],
        forbidden: &[],
    },
    LabeledCase {
        sql: "COMMIT RELEASE",
        expect: Expect::Accept,
        required: &[&TRANSACTION_RELEASE],
        forbidden: &[],
    },
    LabeledCase {
        sql: "RELEASE s",
        expect: Expect::Accept,
        required: &[&TRANSACTION_SAVEPOINTS, &RELEASE_SAVEPOINT_KEYWORD_OPTIONAL],
        forbidden: &[],
    },
...
mod tests {
    ...
    fn feature_subflags_are_explicitly_enumerated() {
        ...
        let TransactionSyntax {
            ...
            begin_transaction_modes: _,
            transaction_savepoints: _,
            set_transaction: _,
            transaction_isolation_mode: _,
            transaction_access_mode: _,
            transaction_deferrable_mode: _,
            start_transaction_isolation_mode: _,
            start_transaction_deferrable_mode: _,
            start_transaction_consistent_snapshot: _,
            transaction_multiple_modes: _,
            transaction_modes_require_commas: _,
            transaction_modes_reject_duplicates: _,
            abort_transaction_alias: _,
            end_transaction_alias: _,
            transaction_release: _,
            transaction_chain: _,
            release_savepoint_keyword_optional: _,
            begin_transaction_mode: _,
            xa_transactions: _,
        } = TransactionSyntax::POSTGRES;
        let UtilitySyntax {
            comment_if_exists: _,
            copy: _,
            copy_into: _,
        ...
        let ShowSyntax {
            ...
            set_value_reserved_words: _,
            set_value_on_keyword: _,
            set_value_null_keyword: _,
            show_tables: _,
            show_columns: _,
            show_create_table: _,
            show_functions: _,
