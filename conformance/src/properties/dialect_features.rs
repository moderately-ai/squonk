...
mod organization_completeness {
    ...
    /// variant so maturity/coverage gates can see it.
    ///
    /// Sub-flag breadth is the ToggleableFeature + LabeledCase system
    /// (`every_gated_subflag_is_required_by_a_labeled_case`) plus the `knob-org` orphan
    /// scan for every bool field under crates/squonk/src.
    #[test]
    fn every_feature_set_axis_has_a_feature_enum_variant() {
        ...
        assert!(ids.contains(&"transaction_syntax"));
        assert!(ids.contains(&"view_sequence_clause_syntax"));
        assert!(ids.contains(&"utility_syntax"));
    }
}
